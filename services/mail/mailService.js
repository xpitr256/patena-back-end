...
function getWorkInProgressMailData(language, workType, workId) {
  ...
  mailData.subject =
    workType === constants.TYPE_DESIGN
      ? translations.mailService.workInProgress.subject.design
      : translations.mailService.workInProgress.subject.analysis;
  mailData.title =
    workType === constants.TYPE_DESIGN
      ? translations.mailService.workInProgress.title.design
      : translations.mailService.workInProgress.title.analysis;
  mailData.description = translations.mailService.workInProgress.description;
  mailData.orderNumber = workId;
  mailData.availableMessage =
    translations.mailService.workInProgress.availableMessage;
...
function getWorkSuccessMailData(language, workType, workId) {
  ...
  mailData.subject =
    workType === constants.TYPE_DESIGN
      ? translations.mailService.workSuccess.subject.design
      : translations.mailService.workSuccess.subject.analysis;
  mailData.title =
    workType === constants.TYPE_DESIGN
      ? translations.mailService.workSuccess.title.design
      : translations.mailService.workSuccess.title.analysis;
  ...
  const resultsLink =
    config.FRONT_END_BASE_URL + "/results?orderNumber=" + workId;
  mailData.resultsUrl = resultsLink;
  mailData.linkMessage =
    translations.mailService.workSuccess.linkMessage +
    '<a href="' +
    resultsLink +
    '" target="_blank">' +
    resultsLink +
...
function getWorkErrorMailData(language, workType, workId) {
  ...
  mailData.subject =
    workType === constants.TYPE_DESIGN
      ? translations.mailService.workError.subject.design
      : translations.mailService.workError.subject.analysis;
  mailData.title =
    workType === constants.TYPE_DESIGN
      ? translations.mailService.workError.title.design
      : translations.mailService.workError.title.analysis;
  mailData.description = translations.mailService.workError.description;
  mailData.orderNumber = workId;
  const contactUsLink = config.FRONT_END_BASE_URL + "/contact";
  mailData.tryAgain =
    translations.mailService.workError.tryAgain +
    ' <a href="' +
    contactUsLink +
    '" target="_blank">' +
    translations.mailService.workError.contactUs +
    "</a>";
...
module.exports = {
  sendContactMail: async function (email, name, message) {
    return new Promise((resolve, reject) => {
      const htmlTemplate = fs.readFileSync(
        "./services/mail/templates/contactMessage.html",
        "utf-8"
      );
  ...
  sendWorkInProgressMail: function (email, language, workType, workId) {
    return new Promise((resolve, reject) => {
      const htmlTemplate = fs.readFileSync(
        "./services/mail/templates/workInProgress.html",
        "utf-8"
      );
  ...
  sendWorkSuccessMail: function (email, language, workType, workId) {
    return new Promise((resolve, reject) => {
      const htmlTemplate = fs.readFileSync(
        "./services/mail/templates/workSuccess.html",
        "utf-8"
      );
  ...
  sendWorkErrorMail: function (email, language, workType, workId) {
    return new Promise((resolve, reject) => {
      const htmlTemplate = fs.readFileSync(
        "./services/mail/templates/workError.html",
        "utf-8"
      );
